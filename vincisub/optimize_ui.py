...
class OptimizeWindow:
    ...
    def needs_model(self):
        return self.items['Realign'].Checked or self.items['ScriptOptimize'].Checked
    ...
    def calculate(self):
        ...
        if ready:
            for i in self.aligned['failed']:
                rows[i]['start']=source['rows'][i]['start'];rows[i]['end']=source['rows'][i]['end']
        return source,rows
    ...
    def preview(self,event=None):
        ...
        try:
            ...
            self.items['ApplyOptimize'].Text=('计算脚本优化' if self.items['ScriptOptimize'].Checked else '计算音频对齐') if pending else '应用并同步'
            self.items['ApplyOptimize'].Enabled=bool(changes) or pending
    ...
    def submit(self,event=None):
        try:
            ...
            if self.needs_model() and (self.aligned is None or self.alignment_key!=self.key(source)):
                ...
                write_json(self.directory/'request.json',dict(rows=source['rows'],timeline=plan,tail=self.items['Tail'].Value if self.items['Realign'].Checked else 0,realign=bool(self.items['Realign'].Checked),reference_script=self.key(source)[4],vocabulary=(lambda v:v['terms'] if v['enabled'] else [])(vocabulary.load(self.jobs.data))))
