...
4. 识别完成后通过 Resolve API 一次写入原始时间线的新 VinciSub 字幕轨，并校验全部字幕位置。窗口保持显示，无菜单点击或检查器操作。
...
- 初次写入新建字幕轨；后续同步刷新本次字幕轨内的字幕片段，保留轨道和其他字幕轨。单条字幕样式会随片段重建，请在校对完成后设置；轨道样式不主动更改。
- 同步前核对全部字幕 ID、文字和时间；若在达芬奇内改过或轨道已锁定，明确拒绝覆盖。失败时尝试恢复原字幕内容，恢复失败会提示检查任务目录的 `edit-backup.json`。
