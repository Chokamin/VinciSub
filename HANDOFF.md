...
- `.venv/bin/python -B -m unittest discover -s tests -v`：59 项通过。
...
- `.venv/bin/python -m scripts.verify_timeline .vincisub/verification/mandarin.mp4`：双音轨、范围、静音筛选、原生生成、真实识别到自动落轨。最新多选完整流程通过：勾选 A1/A2 并刷新保留选择，生成两条字幕，位于相对时间线 1.0–3.4s 与 3.4–4.6s，原始时间线身份不变、无渲染和音频输出；新增选中字幕、编辑文字、保存同步、字幕轨数量不增及关闭窗口回收验证。
